...
function run() {
  ...
  const jsonOutPath = path.resolve(__dirname, "../src/lib/generatedReferencesContent.json");
  const tsOutPath = path.resolve(__dirname, "../src/lib/generatedReferencesContent.ts");
  ...
  fs.writeFileSync(jsonOutPath, JSON.stringify(jsonData, null, 2), "utf-8");
  console.log(`[sync-publications] Successfully wrote ${jsonOutPath} (${(fs.statSync(jsonOutPath).size / 1024).toFixed(1)} KB)`);
...
`;
  ...
  console.log(`[sync-publications] Successfully wrote ${tsOutPath} (${(tsContent.length / 1024).toFixed(1)} KB)`);
}
