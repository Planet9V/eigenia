Subject: Workshop Notes & Proposed Program structureDate: 1 Sept 2026

Author: J. McKenney, Tetrel (jim.mckenney@tetrelsec.com

# This document is an informal follow-up to the late August 2026 workshop in Austin. It captures my core observations, key takeaways, and critical commercial and engineering success factors, then proposes a concrete program of work.

Challenge: Frontier AI model weights are $500M+ assets that must be defended against sovereign-tier extraction. Anthropic needs hardware-enforced zero trust that protects weights even if the host CPU or facility network is compromised. AMD needs a structured way to deliver trusted silicon and rack architectures without derailing multi-year tape-out schedules.

Looking Forward: Coordinating rapid software deployment, multi-year silicon hardware and silicon engineering cycles requires a shared baseline. This proposed program of work couples a Uniform Platform Security Document (UPSD) with a Dual-Pacing V-Model Program—locking critical protections (such as Annex 7) into current N-1 platforms (Helios/Venice) today, while locking in the architecture for Gen 6 zero-trust silicon tomorrow.
...
Part 1: Workshop Follow-upObservations(J.Mckenney)
...
Anthropic ASL-3 / ASL-4 RSP Requirement

Silicon as the Non-Negotiable Trust Boundary: Anthropic established that chip architecture is an immutable security perimeter, requiring hardware-enforced zero-trust isolation where GPUs distrust host CPUs, operating systems, and baseboard management controllers (BMCs) to protect proprietary model weights and custom inference kernels
...
Clarity on N-1 vs. Gen 6: Anthropic needs to know what exact protections apply to the clusters being deployed next quarter versus what is deferred to the 6-year silicon roadmap.

Constraints on cadence: AMD Hardware teams cannot move at the speed of software sprints, and software teams cannot wait for 5-year silicon tape-outs

Uniform Platform Security Document (UPSD) Baseline: A proposed "single source of truth" for all Anthropic deployments. The proposed "UPSD" could be used for AMD's platform to serve as the authoritative single source of truth governing verified production security features, hardware configurations, and active procurement contracts

Machine-Speed Automated Falsification: The workshop discussed a "Claude AI-augmented vulnerability discovery, testing and validation" process, that would enable the transition from manual PSIRT cycles to automated testbenches using Claude-generated exploit hypotheses paired with hardware-in-the-loop (HIL) emulation. This includes testing hypotheses at  machine speed to meet the pace of model development and deployment, AI-based adversaries and the scaling volume of reported vulnerabilities and exploits.  This capability  requires increased transparency of bill of materials and emulation platforms of Anthropic used  products and platforms for Anthropic  to co-test with AMD

Open Platform Firmware (OpenSIL) Migration:  Potential replacement of proprietary AGESA and BIOS binary blobs with open-source OpenSIL on coreboot, cutting maintenance overhead by up to 80% and enabling transparent boot code inspection

Independent 6-Site Manufacturing HSM Audit: For Supply and Value Chain security for the full manufacturing process, A high degree of confidence of provenance and integrity is required to support high value Model weights and IP:  The formalization of the manufacturing process into an auditable standard across each of AMDs  global HSM provisioning sites to map key injection flows and prepare for Gen 6 on-die asymmetric key generation and support flow down contractual obligations to AMD Value and Supply chain partners.

Supply Chain Transparency & EU CRA Alignment: The partnership requires AMD alignment to Cyber Resilience Act (CRA) for  products provided to Anthropic, To support this  AMD products could improve  Supply Chain Transparency   provide machine-readable M2M schema (HBOM/CBOM/SBOM/OBOM) This could be expanded  with the development of an open coordinator for the supply chain with multi vendor support to satisfy EU Cyber Resilience Act requirements and support a more transparent ecosystem
...
(J.Mckenney)Observed Required Critical Capabilities from workshop discussions
...
Machine-Speed Falsification to enable Claude-driven testbenches to proactively find and validate security vulnerabilities before attackers

Annex 7 Contract Terms bind AMD and ODMs to verifiable security deliverables

## Key Success Factors

(J.Mckenney)Potential success factors derived from workshop discussions

SOW Funding Structure: Structuring this engagement around rolling quarterly SOWs tied to formal V-Model and  engineering gates allows AMD to fund dedicated headcount (systems assurance, firmware engineers) without committing to open-ended R&D liabilities

EU CRA as a Market Enabler: Turning the September 11, 2026 EU Cyber Resilience Act (CRA) compliance into an automated M2M coordinator repository gives AMD a major competitive advantage across European and global hyperscale deployments
...
Proprietary Firmware Blobs: Closed-source platform initialization routines (such as legacy AGESA and BIOS blobs) prevent operators from auditing the silicon root of trust and inspecting early boot execution sequences.
...
Fast-Paced V-Cycle (Horizon 1, 6 months): Focuses on N-1 platforms (Helios / Venice), surveying current key provisioning flows, delivering the UPSD source of truth, and locking Annex 7 contract terms
...
Horizon 2 (Medium) | 18 Months | [CRA Mandate] | [DEXPI Schema] | [OpenSIL/Claude HIL] | 
...
Machine-Speed Falsification (Continuous Verification)  Replace manual PSIRT triage with Claude-driven exploit generation and hardware-in-the-loop (HIL) emulation.

Open Platform Initialization (Cradle-to-Grave Provenance)  Phase out proprietary AGESA/BIOS blobs in favor of OpenSIL, coreboot, and 6-site HSM state surveys.
...
Hardware Memory Firewalling: High Bandwidth Memory (HBM) is isolated from host physical memory mapping. On $N-1$ platforms (Helios / Venice), host DMA access is constrained via firmware access control tables; next-generation silicon (Gen 6) introduces on-die hardware memory firewalls.
...
Zone 2: Compute Tray & Untrusted Host | 3 | EPYC CPUs, Host OS/VMM, OpenSIL/Coreboot, Node BMC (OOBM). Linked via Conduit C-21 (Memory Firewall).
Zone 1: Silicon & Weight Isolation | 4 | Instinct MI455X Accelerators, HBM3e, Caliptra RoT, Locked Kernel Enclave.
[/TABLE]
...
OpenSIL and Coreboot Adoption: Replace legacy, closed platform initialization blobs (such as AGESA) with open-source OpenSIL on coreboot. This cuts long-term firmware maintenance overhead by up to 80% and allows operators to inspect boot code directly
...
Adopting this framework will structure AMD/Anthropic engineering, procurement, and commercial contracts around three parallel execution horizons funded through rolling quarterly Statements of Work (SOWs):
...
Horizon 2 | Mid-Term: Q3 - Q6 |  | EU CRA Compliance, DEXPI 2.0/CycloneDX, Open Coordinator, Claude Exploit Engine, OpenSIL/Coreboot | 
...
Goal: Secure currently deployed N-1 platforms (Helios MI455X trays and Venice SP7 hosts), establish the authoritative UPSD source of truth, and audit manufacturing key management

Core Activities:

Complete current-state surveys across AMD's 6 global EDS HSM sites to map key injection flows and phase out shared HMAC secrets
...
Replace closed AGESA firmware blobs with OpenSIL on coreboot for Venice host nodes

Integrate the Claude automated exploit hypothesis engine with hardware-in-the-loop (HIL) testbenches
...
Semiconductor Manufacturers (AMD, etc.): Publish silicon root-of-trust specifications, DICE certificate hierarchies, OpenSIL platform initialization code, and line-rate encryption specifications

Frontier AI Developers (Anthropic, etc.): Deliver machine-readable RSP verification suites, threat hypotheses, non-token egress constraints, and automated model weight protection benchmarks
...
Bi-Weekly Technical Syncs (AMD / Anthropic / Tetrel): Review interface boundaries, hardware telemetry limits, and open-source OpenSIL bring-up progress
...
Automate Ingestion of physical cooling loops and energy systems directly into simulation engines (such as the proposed "Claude-Assisted Automated Exploit & Falsification Engine"  without manual redrawing or fixed release packages.
...
AGESA: AMD Generic Encapsulated Software Architecture — Legacy closed-source initialization library for AMD processors, being phased out in favor of open-source OpenSIL.

ASL-3 / ASL-4: AI Safety Level 3 / 4 — Anthropic Responsible Scaling Policy tiers requiring hardware-enforced protection against state-sponsored model weight theft and CBRN capability misuse.
...
DPU: Data Processing Unit — Programmable network accelerator (e.g., AMD Pensando) providing line-rate hardware packet filtering, encryption, and telemetry enforcement.

EDS: Enterprise Distribution Server — AMD manufacturing key provisioning system distributed across six global factory Hardware Security Module (HSM) sites.
...
MI455X: AMD Instinct MI455X Accelerator — AMD frontier accelerator packaging multi-chiplet GPUs and High Bandwidth Memory on an advanced packaging interposer.
...
OpenSIL: Open-Source Silicon Initialization Library — AMD open-source architecture that decouples silicon initialization from host firmware, allowing transparent auditability and native integration with coreboot.
...
ROCm: Radeon Open Compute Platform — AMD open-source software development platform and runtime for GPU-accelerated computing and machine learning.
...
RSP: Responsible Scaling Policy — Anthropic operational framework specifying strict safety, security, and containment thresholds (ASL levels) as AI model capabilities scale.
...
Venice / Verano: AMD EPYC Server Processors — Next-generation AMD server CPUs hosting compute node hypervisors, memory controllers, and PCIe root complexes.