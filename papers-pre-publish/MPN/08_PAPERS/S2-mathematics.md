...
| Status | Draft, revision 4, after Skeptic, Constraint Guardian and User Advocate reviews and the arbitration of 13 September 2026. Eleven of the arbitration's twelve conditions are applied; the twelfth, the word ceiling, is open and is the last line of the length field. **The programme's standing caveat on its empirical position was added as section 0 on 14 September 2026**, with three sentences marking, where each is made, the claims that turn on a choice nobody has taken |
...
## 0. A standing note on evidence, which nothing proved here waits on

**No listener has been asked anything, and every empirical claim in this programme is untested.** The 31,078 scored rows and the 232 annotated frames are outputs of the system under study; the listening pack is built, blinded, published and sent to nobody; and the two earlier listener studies are not citable, their stimuli having come from an unseeded generator [16].

**Proofs are proofs and are unaffected.** Lemma 1, Lemma 2, Theorem 1 and Theorem 2 would stand unchanged if every listening result came back null, and so would the four results this paper establishes by inspection: section 3.1's eight dimensions in nine named coordinates, section 4.2's finding that the constraint manufactures the correlation A3 offers as evidence, section 4.4's audit of the register instrument off the state space on 44.8 per cent of frames, and section 4.5's incompatibility of the simplex with the Borromean link. None of them is a claim about listeners, so no listener can touch them.

**That is the whole of the reassurance and this note claims no more.** A null return would fall entirely on the list section 9 already gives of what this paper assumes and does not establish: that the nine coordinates are the right coordinates, that the registers admit magnitudes at all, that the simplex rather than the cube is their domain, and that the coordinates are interval-scaled. Theorems about a construction say what follows from the construction. They do not say the construction describes anything.
...
The binding constraint is not capacity but reach. Of the eight state dimensions, trauma reaches dynamics and the harmonic coordinate; entropy reaches tempo; the two register degrees of freedom reach mode; and **the four DISC coordinates reach only timbre**, which is the one factor the theory has not specified. That trauma is what reaches the harmonic coordinate is itself conditional: S3 re-expresses the harmonic position as a chain position given by a state scalar the theory has not named, which is open and is the author's, so which coordinate reaches that channel moves with a function nobody has written. So four of the eight dimensions currently survive into an unspecified channel, and the fibre structure of $\Phi$ cannot be computed until $T$ is. What can be said without it is that on the composer path of section 6.5 the registers are hard-coded, so the two register dimensions reach nothing at all, and entropy does not arrive, so the recoverable state on that path is one-dimensional: everything audible is a function of trauma.

That is the number the programme needs and it should be in front of any listening study. A study that asks listeners to recover a register from a cue produced on that path is asking them to recover a constant. Section 10 carries the completion of this analysis as an action on S3, once $T$ exists. The completion turns on one number this paper cannot supply either: how finely a listener resolves the timbre space, which is unmeasured and which Part D of the listening pack is built to fix, so the count of characters that channel can carry is conditional on a figure nobody yet has.
