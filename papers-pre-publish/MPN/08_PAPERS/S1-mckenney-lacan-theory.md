...
| Status | Draft, revision 3, after Block A arbitration and a Skeptic review of revision 2. **The programme's standing caveat on its empirical position was added as section 0 on 14 September 2026**, together with the sentences marking, at the point each is made, the claims that turn on a choice nobody has taken |
...
## 0. The standing caveat, before any claim below

**No listener has been asked anything. Every empirical claim in this paper is untested.** Everything the programme holds was produced by the programme: 31,078 scored rows and 232 annotated frames, both outputs of the system this paper theorises. The listening pack is built, blinded and published, and it has been sent to nobody. Two earlier listener studies exist and neither is citable, because their stimuli came from an unseeded generator and are not reproducible; section 11 reports both.

**The register already sorts the assertions into three kinds, and the caveat falls hard on one of them.** The classification is taken from the companion register and is not reinvented here. **True by construction**, and so not the kind of thing a listener could confirm or refute: A1's nine named coordinates and their unit bounds, which stipulate what the state is; A3's simplex, which a line of arithmetic imposes rather than observes; A7's five named fragmentation stages, which are a naming of an existing practice; and A11's composition claim, which is a design commitment about how the functions are assembled. **Analytically testable, and tested**, by algebra or by reading the source rather than by anybody's ear: A8, whose collinearity is an identity in the weights, confirmed at $r = 0.9191$ over the scored rows and floored at 0.841 over every correlation the inputs could have, which is why section 8.2 rests on the algebra and treats the frames as illustration; A6 and A9, each falsified by an enumeration of what the implementation does and does not reach; and A11's determinism half, falsified by counting unseeded calls. **Mapping claims that need a listener and have none**: A2, A4, A5, the perceptual half of A7, the composition half of A11, and the whole of B1 to B5.

**Nothing in this paper is weakened by saying so.** A claim stated as conditional on a test that exists is stronger than the same claim stated flatly, because a reader can see what would change it. Where a section makes a claim that turns on a choice nobody has taken, it says so at that point rather than leaving it to section 11.
...
The nine components are the working state. Behind them the theory defines a wider personality space of twenty-four dimensions, formed from the four DISC dimensions, the five Big Five dimensions, the three Dark Triad dimensions and twelve named cognitive biases. The twelve are a selection made for that space and they are not the bias layer of section 9, which carries thirty and is a separate object indexed per turn rather than per person. The relation between the two is not settled: either the twelve are a named subset of the thirty, in which case the paper owes a list, or the wider vector should carry the thirty and become forty-two dimensional. Section 9.1 settles the count for the layer and does not settle this. That space is standardised per dimension, reduced by principal or independent component analysis to roughly eight factors said to capture the large majority of variance, and normalised to the unit sphere.
...
Two of the five component functions are named here and are not yet functions of anything definite. $f_{\text{timbre}}$ waits on a timbre space whose perceptual resolution is unmeasured, and which Part D of the listening pack is built to fix; $f_{\text{harmony}}$ is re-expressed in S3 as a position given by a state scalar the theory has not named, which is open and is the author's. Both are stated here as components of $\Phi$ and neither can be checked against anything until those two are settled.
...
The stage ladders are declared a priori at even fifths on both scalars rather than fitted to the corpus, which gives fifteen of the twenty-five level pairs against ten for the pair replaced. Full dissolution requires near-total disorder with almost no weight and is rare by construction; no frame in the library reaches it.
...
The paper therefore records A4 as an open documentation defect rather than settling it by argument. The way to settle it is by ear, against the seven modes over a fixed harmonic bed, and the assignment that results becomes the single normative table in S3. Everything this section says about the four tables is conditional on that choice, and the choice is now under test rather than merely open: Part A of the published listening pack asks a panel which scale goes with which predicament and whether the answer changes at extremity, which settles the assignment and the second stage together, and no response has been returned or scored.
...
**B4. The two rendered layers take channels that do not compete.** Profile takes timbre and register, set once per persona; state takes dynamics and mode. **Amended 15 September 2026.** B4 was stated as a three-layer claim whose third clause gave bias the harmonic relation between staves. Decision A of 14 September took BL-8 as option (e), under which the bias layer occupies no musical channel and renders nothing, and the two texts contradicted each other; the arbitration of 14 to 15 September recorded the contradiction and left it to the author, who has struck the clause. The bias layer is not a third channel and makes no claim on the budget. One thing the amendment does not touch: $f_{\text{harmony}}$ in section 6's definition of $\Phi$ is a coordinate of the state mapping and is a different object from the between-stave harmonic relation, so it stays, and naming $f$ remains open. The reason is a channel budget rather than a preference: a stave carries only so many independent quantities before a reader recovers none of them. The working notes put that at two to three per stave, which would put three layers over the limit, and that number is an estimate rather than a measured figure; S3 owes either its derivation or the recovery study that replaces it. Moving bias between the voices is not an evasion, it follows from B3.
