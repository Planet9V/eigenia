...
### B4. The two rendered layers are carried on channels that do not compete

**AMENDED 15 September 2026 by the author.** The between-stave clause is struck. B4 was stated as a three-layer claim in which bias took the harmonic relation between staves; decision A of 14 September took BL-8 as option (e), under which the bias layer occupies no musical channel and renders nothing on any surface, and the two texts contradicted each other. The arbitration of 14 to 15 September recorded the contradiction as its decision 9.1 and declined to resolve it, restating an assertion being an amendment to the calculus and the calculus being the author's. The author has amended it. The clause goes; B4 is now a claim about the two layers that do render, and its falsifier narrows accordingly. What is below is the amended assertion. The struck text is preserved at the end of this entry so that a reader of an earlier revision can see what moved.

**Claim.** Profile and state are simultaneously legible because they are assigned to channels that do not compete: the stable profile takes timbre and register, set once per persona; the moving state takes dynamics and mode. The bias layer is not a third channel and makes no claim on the budget, because under BL-8 option (e) it renders nothing.

**Formal content.** Channel assignment is fixed, declared, and counted against the per-stave budget of two to three independent quantities. There is no between-stave channel, so nothing in this assertion is exempt from that budget. The budget applies per stave and is therefore independent of how many personas a score carries: a score of fourteen personas is fifteen staves each held to the same bound, which is why B4 constrains legibility per stave and says nothing about cast size. What caps cast size is the timbre channel's capacity, which is a separate measured question.
...
**How it could be wrong.** Listeners cannot recover either layer above chance. Or the two layers mask each other, so that recovering one degrades recovery of the other. Or the budget of two to three is itself wrong, in either direction, which is the disjunct the amendment makes decisive: with the between-stave escape removed there is nothing left in B4 that is exempt from the budget, so a measured budget that differs from two to three falsifies the assertion directly rather than moving a layer out of its way. MPN-S5 already shows the surfaces oversubscribed against it on either count. **The amendment makes B4 easier to falsify, not harder, and that is the point of making it.**

**Test.** A recovery study, one layer at a time, then both together, against the two-to-three channel bound. The bound itself is measured in the same study rather than assumed, which is what the third disjunct above now requires.

**Struck by the amendment, preserved for comparison.** The original claim read: "and bias, which lives between speakers, takes the harmonic relation between staves rather than any channel on a single stave"; the original formal content added "The bias channel is a between-stave quantity and therefore does not consume a within-stave channel"; and the original falsifier's third disjunct read "Or the between-stave channel turns out to be a within-stave channel in disguise, in which case the budget is exceeded after all". Note for any reader tracing $f_{\text{harmony}}$: that is a coordinate of $\Phi$, the state mapping, and is a different object from the between-stave harmonic relation struck here. The amendment does not touch it, and naming $f$ remains open as item S3-12.
