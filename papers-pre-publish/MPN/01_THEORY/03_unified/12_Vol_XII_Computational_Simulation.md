...
## Expert 3: The Physicist (Entropy)
...
If a society consists of 90% **Obsessional Nodes** ($S_{gate} \approx 0$):

* **Macro-State**: Stability (Low Variance).
* **Vulnerability**: Total inability to process Semantic Shocks.
* **Shock Response**: When a Black Swan hits, the graph shatters because it cannot update its weights. (Brittle Failure).

If a society consists of 90% **Hysteric Nodes**:

* **Macro-State**: Chaos (High Variance).
* **Vulnerability**: No historical continuity. Revolution every Tuesday.

## 3.3 The Optimal Mix (The Omega Point)

The **Psychohistory Engine** optimizes the population mix to maximize Integrated Information ($\Phi$).

* **The Mix**:
  * **70% Flexible Nodes** ($z_t \approx 0.5$): To process information.
  * **20% Obsessional Nodes**: To maintain Law/Memory ($S_{gate} \to 0$).
  * **10% Hysteric Nodes**: To act as alarm systems (Radicals).
