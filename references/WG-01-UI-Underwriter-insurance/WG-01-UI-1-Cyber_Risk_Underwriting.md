...
### 2.2 Penetration Testing as an Independent Control Check

For entities operating within critical infrastructure sectors, baseline external scanning is entirely insufficient. Insurers increasingly mandate third-party penetration testing as a hard requirement for policy eligibility and favorable premium calculation. Penetration testing exercises an organization's existing security controls against simulated attack vectors, from the perspective of an ethical hacker. The asymmetry in what a test result carries is worth stating plainly to an underwriter. A test that gets in demonstrates a defect. A test that does not get in bounds the tester's time, scope and skill, not the organisation's security. A comprehensive penetration test report therefore provides underwriters with dated, scoped evidence of a firm's cyber hygiene at the moment of test, which directly influences policy eligibility, coverage limits, and standard terms.
...
Underwriters bridge the gap between the baseline technical price and the final commercial price by applying specific loading factors (surcharges) and discount factors (credits) based on the organization's independently assessed cybersecurity posture. The telemetry data and penetration test results gathered during the assessment phase are what supply that assessment, and they feed directly into these modifiers.
...
In addition, overall cyber pricing scales directly with annual revenue, though the scaling is non-linear. As demonstrated in Table 2, premium increases tend to flatten at higher revenue tiers because massive organizations typically possess exponentially greater internal risk management resources.
