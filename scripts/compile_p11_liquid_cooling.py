...
For a 100 MW facility, deploying analog thermal switches ($C_{\text{controls}} = 180,000\text{ USD}$) reduces annual loss expectancy from $8,450,000\text{ USD}$ down to $125,000\text{ USD}$, delivering a modelled $\text{ROSI} = 4,525\%$. Modelled, not measured: the arithmetic is exact, but both loss expectancies are engineering estimates for a reference 100 MW hall chosen by the authors, and neither is drawn from claims experience at an operating site. An underwriter should substitute the loss figures for the plant in front of them and recompute before the number carries any weight. EN 50126 and IEC 62443 compliance requires that the provenance of every safety-related component be recorded and auditable, while Caliptra 2.0 silicon roots of trust, DICE identities, and OpenSIL initializers eliminate common-cause firmware exploitation.
...
| **Lloyd's Y5381 War Exclusion** | Disputed claims during nation-state cyber campaigns; denied coverage. | Affirmative cyber-physical coverage granted; war exclusion waived. | Physical air gaps, traced end to end at commissioning and re-checked at each proof-test interval, give the loss adjuster a physical record of containment. |
| **Portfolio Accumulation Loading** | 40% capital surcharge to protect against correlated cluster-wide cooling trip. | 0% accumulation surcharge; racks confirmed hydraulically decoupled by a commissioning flow test on every secondary loop. | Eliminates systemic capital loadings across multi-campus portfolios. |
